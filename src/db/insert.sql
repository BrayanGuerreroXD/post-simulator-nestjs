...
CREATE OR REPLACE FUNCTION get_comment_descendants_count(comment_id BIGINT)
RETURNS INT AS $$
DECLARE
...
BEGIN
    ...
    SELECT COUNT(*) - 1 INTO descendants_count FROM descendants;

    
    IF descendants_count > 0 THEN
        RETURN descendants_count - 1;
    ELSE
        RETURN 0;
    END IF;
END;
...
-- Get a comment nested by its first answer and so on with this one.
...
BEGIN
    ...
    child_count := (SELECT CASE WHEN get_comment_descendants_count((comment->>'id')::BIGINT) > 0 THEN get_comment_descendants_count((comment->>'id')::BIGINT) ELSE 0 END);
    ...
    IF children IS NOT NULL THEN
        SELECT build_comment_tree(comments, (children->0->>'id')::BIGINT) INTO first_reply;
    ELSE
        first_reply := NULL;
    END IF;
